...
项目使用 `main` 分支锁定的 NVIDIA CUDA 版 PyTorch。安装并验证环境：
...
`torch.version.cuda` 应为非空 CUDA 版本，设备名应为 NVIDIA GPU。下列命令中的
`--device cuda` 使用 NVIDIA CUDA 加速。
...
对应权重为 `checkpoints/stream_v5_deployment_cuda.pt`。9.872% 不能外推为任意
...
旧版参考权重为 `checkpoints/stream_cuda_warmup_cosine.pt`。数据和权重均被
