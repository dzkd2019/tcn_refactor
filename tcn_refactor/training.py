...
def configure_cuda_for_causal_tcn(device: torch.device) -> None:
    ...
    模型采用“内置 padding 后裁右侧”的实现，既保持因果性，又避免旧版显式
    单边 ``F.pad`` 在本机 cuDNN 上的异常退出。因此可以安全启用 cuDNN，
    获得完整 GPU 加速。
    """
    if device.type == "cuda":
...
def train_stream_model(data_path: str, checkpoint_path: str, *, validation_path: str | None = None,
                       ...
                       max_offset_s: float = 6.0, device: str = "cpu",
    ...
    dataset = StreamWindowDataset(data_path, config=config, max_offset_s=max_offset_s)
    # 正式实验使用独立验证文件；缺省时才退回按场景的 80/20 划分，便于快速试验。
    if validation_path is None:
        train_set, val_set = _split_by_scenario(dataset)
    else:
        train_set = dataset
        # 部署验证集也模拟检测延迟，避免只按理想对齐窗口选出不鲁棒模型。
    ...
    for epoch in range(epochs):
        ...
        print(f"epoch={epoch + 1:03d} loss={total_loss / seen:.5f} "
