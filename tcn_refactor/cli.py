...
def _parser() -> argparse.ArgumentParser:
    ...
    generate.add_argument(
        "--duration-s", type=float, default=180.0, help="每个场景时长（秒）"
    )
    ...
    train.add_argument(
        "--val-data", help="独立验证场景 .npz 路径；正式训练建议提供"
    )
    train.add_argument(
        "--checkpoint", required=True, help="模型 checkpoint 输出路径"
    )
    train.add_argument("--epochs", type=int, default=20, help="训练轮数")
    train.add_argument("--batch-size", type=int, default=32, help="每批窗口数")
    train.add_argument(
        "--lr", type=float, default=5e-4, help="AdamW 学习率；续训时建议降低"
    )
    train.add_argument(
        "--warmup-epochs", type=int, default=0, help="线性 warmup 轮数"
    )
    train.add_argument(
        "--min-lr",
        type=float,
        help="余弦退火最低学习率；不提供则保持固定学习率",
    )
    ...
    evaluate.add_argument(
        "--start", type=int, default=0, help="从第几个场景开始回放"
    )
    evaluate.add_argument("--limit", type=int, help="本次最多回放多少个场景")

    window_eval = command.add_parser(
        "window-eval", help="在已对齐独立测试窗口上评估模型"
    )
    window_eval.add_argument("--data", required=True, help="测试场景 .npz 路径")
    window_eval.add_argument(
        "--checkpoint", required=True, help="已训练模型路径"
    )
    window_eval.add_argument(
        "--batch-size", type=int, default=8, help="评估 batch 大小"
    )
...
def main() -> None:
    ...
    if args.command == "generate":
        ...
        result = replay(
            args.data,
            StreamSession(model, config=config, device=args.device),
            start=args.start,
            limit=args.limit,
        )
        print(
        ...
        result = evaluate_time_points(
            model, loader, model.head[-1].weight.device, config
        )
        print(
            "独立窗口评估："
            + "，".join(f"{key}={value:.3f}" for key, value in result.items())
