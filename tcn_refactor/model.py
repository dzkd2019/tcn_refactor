...
class StreamingTCN(nn.Module):
    ...
    def __init__(
        ...
        for index, current in enumerate(channels):
            layers.append(
                TemporalBlock(previous, current, 2**index, dropout, kernel_size)
            )
            previous = current
        self.backbone = nn.Sequential(*layers)
        self.head = nn.Sequential(
            nn.Conv1d(previous, previous, 1),
            nn.ReLU(),
            nn.Conv1d(previous, 1, 1),
        )
        self.context_head = None

        """
        最终原始输出为：z = z_tcn + z_context，其中 z_tcn 是 TCN 主干输出，z_context 是 context head 输出。
        它相当于一个从原始特征直接到输出的快捷分支。
        ...
        可以理解为：TCN 学时序关系，context head 学当前特征的即时修正。
        """
        if use_context_head:
            ...
            # 让 context head 的初始输出为 0，避免训练初期对 TCN 主干的梯度干扰。此时 z = z_tcn
        ...
        levels = (
            () if calibration_levels_ppm is None else calibration_levels_ppm
        )
        ...
        if output_mode == "softplus":
            ...
            self.head[-1].bias.data.fill_(
                math.log(occupancy / (1.0 - occupancy))
            )
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        if self.output_mode == "sigmoid":
            ...
            root_concentration = occupancy / (
                self.langmuir_k * (1.0 - occupancy)
            )
        ...
        if (
            not self.training
            and self.quantize_in_eval
            and self._calibration_levels.numel()
        ):
