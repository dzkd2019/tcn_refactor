...
class CoreContractTests(unittest.TestCase):
    ...
    def test_cuda_device_rejects_non_rocm_torch(self) -> None:
        """GPU 训练必须拒绝 CPU 或 CUDA 构建，避免使用错误的 PyTorch。"""
        with patch.object(torch.version, "hip", None), patch.object(torch.version, "cuda", "13.0"):
            with self.assertRaisesRegex(RuntimeError, "需要 PyTorch ROCm 构建"):
